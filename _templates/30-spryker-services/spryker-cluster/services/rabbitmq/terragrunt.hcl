...
locals {
  spryker_service = read_terragrunt_config(find_in_parent_folders("config/spryker-services/ecs_service-rabbitmq.hcl"))
...
inputs = {
  ...
  volumes = {
    rabbitmq_data = {
      target       = dependency.efs.outputs.efs_file_system_id.rabbitmq
      type         = local.spryker_service.locals.volumes.rabbitmq_data.type
      efs_root_dir = local.spryker_service.locals.volumes.rabbitmq_data.efs_root_dir
    }
  }
