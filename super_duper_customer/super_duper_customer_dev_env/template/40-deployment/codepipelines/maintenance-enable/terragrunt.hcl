...
inputs = {
  ...
  pipeline_type              = local.spryker.locals.scheduler_type == "ecs" ? local.codepipelines_ecs.locals.codepipelines_config.maintenance_enable.pipeline_type : local.codepipelines_ec2.locals.codepipelines_config.maintenance_enable.pipeline_type 
  run_stages                 = local.spryker.locals.scheduler_type == "ecs" ? local.codepipelines_ecs.locals.codepipelines_config.maintenance_enable.run_stages : local.codepipelines_ec2.locals.codepipelines_config.maintenance_enable.run_stages
  restart_services           = []
